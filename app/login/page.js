...
import { loginStaff } from "@/app/lib/authService";
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (result.success) {
      ...
      localStorage.setItem("centerName", result.staffData.centerName);
      localStorage.setItem("centerId", result.staffData.centerId);
      localStorage.setItem("staffName", result.staffData.fullName);
      localStorage.setItem("staffId", result.staffData.staffId);
    ...
    <section className="grid w-full max-w-6xl items-center gap-10 lg:grid-cols-[1fr_0.9fr]">
      ...
      <div className="hidden lg:block">
        <div className="glass-card rounded-[3rem] p-10">
          ...
          <h1 className="max-w-xl bg-gradient-to-br from-[#006a61] via-[#13b7a7] to-[#0058be] bg-clip-text text-6xl font-bold leading-tight tracking-[-0.06em] text-transparent">
            Welcome back to HealthGrid AI.
          </h1>

          <p className="mt-6 max-w-lg text-lg leading-8 text-[#3c4947]">
            Login to submit daily PHC/CHC reports, update stock details, and help
            admins detect urgent health center risks faster.
          </p>

          <div className="mt-10 grid grid-cols-2 gap-4">
            <div className="rounded-[1.7rem] border border-white/70 bg-white/35 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_14px_35px_rgba(0,106,97,0.1)] backdrop-blur-3xl">
              <p className="text-sm font-bold uppercase tracking-[0.12em] text-[#006a61]">
                Reports
              </p>
              <p className="mt-2 text-3xl font-bold text-[#01201f]">Daily</p>
            </div>
            ...
            <div className="rounded-[1.7rem] border border-white/70 bg-white/35 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_14px_35px_rgba(0,106,97,0.1)] backdrop-blur-3xl">
              ...
              <p className="mt-2 text-3xl font-bold text-[#01201f]">Smart</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card relative mx-auto w-full max-w-md rounded-[3rem] p-7 md:p-9">
        ...
        <div className="relative">
          ...
          <div className="mt-7 space-y-3 text-center text-sm font-medium text-[#3c4947]">
            <p>
              New staff?{" "}
              <a
                href="/signup"
                className="font-bold text-[#006a61] transition hover:text-[#13b7a7]"
              >
